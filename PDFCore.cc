...
int PDFCore::loadFile2(PDFDoc *newDoc) {
  ...
  while (pages->getLength() > 0) {
    delete static_cast<PDFCorePage *>(pages->del(0));
  }
...
void PDFCore::update(int topPageA, int scrollXA, int scrollYA,
  ...
  if (continuousMode) {
    ...
    page = static_cast<PDFCorePage *>(pages->get(0));
  }
  topPage = topPageA;
  ...
  if (continuousMode) {
    ...
    while (pages->getLength() > 0 &&
	   (static_cast<PDFCorePage *>(pages->get(0)))->page < pg0) {
      delete static_cast<PDFCorePage *>(pages->del(0));
    }
    i = pages->getLength() - 1;
    while (i > 0 && (static_cast<PDFCorePage *>(pages->get(i)))->page > pg1) {
      delete static_cast<PDFCorePage *>(pages->del(i--));
    }
    j = pages->getLength() > 0 ? (static_cast<PDFCorePage *>(pages->get(0)))->page - 1
                               : pg1;
    for (i = pg0; i <= j; ++i) {
      rot = rotate + doc->getPageRotate(i);
      if (rot >= 360) {
    ...
    j = (static_cast<PDFCorePage *>(pages->get(pages->getLength() - 1)))->page;
    for (i = j + 1; i <= pg1; ++i) {
      rot = rotate + doc->getPageRotate(i);
      if (rot >= 360) {
	rot -= 360;
  ...
  for (i = 0; i < pages->getLength(); ++i) {
    page = static_cast<PDFCorePage *>(pages->get(i));
    j = 0;
    while (j < page->tiles->getLength()) {
      tile = static_cast<PDFCoreTile *>(page->tiles->get(j));
      if (continuousMode) {
	...
	delete static_cast<PDFCoreTile *>(page->tiles->del(j));
      } else {
	++j;
      }
    }
...
void PDFCore::addPage(int pg, int rot) {
  ...
  for (i = 0;
       i < pages->getLength() && pg > (static_cast<PDFCorePage *>(pages->get(i)))->page;
       ++i) ;
  pages->insert(i, page);
}
...
void PDFCore::needTile(PDFCorePage *page, int x, int y) {
  ...
  if (!page->text) {
    if ((textOut = new TextOutputDev(NULL, true, false, false))) {
...
bool PDFCore::gotoNextPage(int inc, bool top) {
  ...
  update(pg, scrollX, scrollYA, zoom, rotate, false, true);
  return true;
}
...
bool PDFCore::gotoPrevPage(int dec, bool top, bool bottom) {
  ...
  if (continuousMode) {
    ...
    scrollYA = (static_cast<PDFCorePage *>(pages->get(0)))->h - drawAreaHeight;
    if (scrollYA < 0) {
      scrollYA = 0;
    }
  } else {
...
void PDFCore::scrollDownNextPage(int nLines) {
  if (!continuousMode &&
      scrollY >= (static_cast<PDFCorePage *>(pages->get(0)))->h - drawAreaHeight) {
    gotoNextPage(1, true);
  } else {
    scrollTo(scrollX, scrollY + nLines);
  }
...
void PDFCore::scrollTo(int x, int y) {
  update(topPage, x, y < 0 ? 0 : y, zoom, rotate, false, false);
}

void PDFCore::scrollToLeftEdge() {
  update(topPage, 0, scrollY, zoom, rotate, false, false);
}
...
void PDFCore::scrollToTopEdge() {
  ...
  update(topPage, scrollX, y, zoom, rotate, false, false);
}
...
void PDFCore::scrollToTopLeft() {
  ...
  y = continuousMode ? pageY[topPage - 1] : 0;
  update(topPage, 0, y, zoom, rotate, false, false);
}
...
void PDFCore::scrollToBottomRight() {
  ...
  update(topPage, x, y, zoom, rotate, false, false);
}
...
void PDFCore::zoomToRect(int pg, double ulx, double uly,
  ...
  update(pg, sx, sy, newZoom, rotate, false, false);
}
...
void PDFCore::zoomCentered(double zoomA) {
  ...
  update(topPage, sx, sy, zoomA, rotate, false, false);
}
...
void PDFCore::zoomToCurrentWidth() {
  ...
  update(topPage, sx, sy, (dpi1 * 100) / 72, rotate, false, false);
}

void PDFCore::setContinuousMode(bool cm) {
  if (continuousMode != cm) {
    continuousMode = cm;
    update(topPage, scrollX, -1, zoom, rotate, true, false);
  }
}
...
GooString *PDFCore::extractText(int pg, double xMin, double yMin,
  ...
  if ((page = findPage(pg))) {
    ...
    textOut = new TextOutputDev(NULL, true, false, false);
    if (textOut->isOk()) {
...
void PDFCore::setReverseVideo(bool reverseVideoA) {
  ...
  update(topPage, scrollX, scrollY, zoom, rotate, true, false);
}
...
void PDFCore::redrawCbk(void *data, int x0, int y0, int x1, int y1,
			bool composited) {
  PDFCore *core = static_cast<PDFCore *>(data);
...
void PDFCore::redrawWindow(int x, int y, int width, int height,
  ...
  for (i = 0; i < pages->getLength(); ++i) {
	...
	clippedRedrawRect(NULL, 0, 0,
			  xDest, yDest,
			  w, (static_cast<PDFCorePage *>(pages->get(i+1)))->yDest - yDest,
			  x, y, width, height, false);
      }
      if (tile->edges & pdfCoreTileLeftEdge) {
	clippedRedrawRect(NULL, 0, 0,
